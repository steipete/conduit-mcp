...
*   **First Use Informational Message:**
    *   The server must maintain a state variable (e.g., a boolean flag, initialized to `false` at server start) to track if it has sent a "first use informational message" during its current runtime session.
    *   On the **very first successful tool request processed by the server in a session**, if the `CONDUIT_ALLOWED_PATHS` environment variable was *not* explicitly set by the user (and thus the server defaulted to using `~:/tmp`), the MCP response for *that first successful tool request* **must** prepend an additional content item to its primary response array (e.g., the `content` array for `read`, or the array of result objects for batched `write` actions, or the array of `EntryInfo` for `list.entries` and `find`). If the tool response is a single object (e.g., `read.diff`, `write.archive`, `list.system_info`), this informational message should be prepended as the first item in a new top-level array, with the original single object response becoming the second item in that array, and the client should be prepared to handle this array structure for the first response.
    *   The server must set its internal "first use message sent" flag to `true` after successfully including this notice, ensuring it is sent only once per server session.
    *   **Structure of the prepended informational message (as a JSON object):**
        ```json
        {
          ...
          "details": {
            "server_version": "1.0.0", // Actual server version
        ...
        ```
    *   The server version and its actual start time **must** be dynamically included. The resolved paths for `~` and `/tmp` **must** also be included.
*   **Security:**
...
*   **Idempotency:**
    *   `write.action: "mkdir"`: If the directory specified (and all parent directories if `recursive: true`) already exists, the operation will succeed without error. No special message is included in the MCP response, but an internal `DEBUG` level log (if internal logging is active) can note this.
    *   `write.action: "touch"`: If the file already exists, its access and modification timestamps are updated to the current time. If it doesn't exist, it is created as an empty file. This operation will always report success if the path is writable.
*   **Logging (Internal Server Logging Only):**
    *   The server **must not** write any operational logs to `stdout` or `stderr`.
    *   It should be implemented with an internal, configurable logging mechanism (e.g., using `pino` that can be configured to write to a file, a logging service, or be a no-op logger by default if not explicitly configured for output).
    *   This internal logger (if its output is enabled via means outside MCP, e.g., for server debugging by an administrator) would record:
        *   At `DEBUG` or `INFO` level (based on `LOG_LEVEL`): Server startup with all active `CONDUIT_*` configuration values (no redaction).
        *   At `DEBUG` level: MCP request received (including `toolName`, `operation`/`action`, and key parameters, but not full raw content to avoid excessive logging).
        *   At `INFO` or `DEBUG` level: Outcome of significant operations (e.g., file written, URL fetched, search completed).
        *   At `ERROR` or `WARN` level: Details of any internal errors or significant issues encountered, along with their corresponding `error_code`.
*   **Webpage Cleaning Pipeline (for `read.operation: "content", format: "markdown"` on URLs):**
    1.  Fetch raw content from the URL.
    2.  Check the `Content-Type` header of the response. If it is not `text/html` or a closely related HTML variant (e.g., `application/xhtml+xml`), then the fallback behavior described in Section 5.1 for `format: "markdown"` on non-HTML URLs is triggered.
    3.  If HTML, parse the content using `jsdom`.
    4.  Extract the main content block from the parsed DOM using `@mozilla/readability`. If `readability.parse()` returns `null` or fails to extract meaningful content, this is considered a failure of the cleaning process, resulting in an `ERR_MARKDOWN_CONTENT_EXTRACTION_FAILED` error for that source.
...
*   **Image Compression (for `read.operation: "content", format: "base64"` on image URLs/files):**
    ...
    *   If the raw image data size (before base64 encoding) exceeds `CONDUIT_IMAGE_COMPRESSION_THRESHOLD_BYTES`, the server **must** attempt to compress the image using `sharp`.
        ...
        *   For PNG: Use optimization options available in `sharp`.
        *   Other formats: Apply suitable lossless or lossy compression available in `sharp`.
    *   If compression is successful, the compressed image data is then base64 encoded. The response **must** include `compression_applied: true` and `original_size_bytes`. If no compression was attempted (e.g., below threshold, or unsupported type for compression by `sharp`), `compression_applied: false`. If compression fails, an error should be logged internally, but the server should proceed to base64 encode the original image and set `compression_applied: false` with an optional `compression_error_note` in the response.
*   **Character Encoding:** All text file reading, writing, and text content from URLs (after decoding HTTP responses according to their `Content-Type` or defaulting to UTF-8 if unspecified) **must** assume, operate with, and produce UTF-8 encoded text.
*   **Date/Time Format:** All timestamps produced by the server and included in MCP responses (e.g., `created_at_iso`, `modified_at_iso`, `server_start_time_iso`) **must** be strings formatted according to ISO 8601 and **must** represent Coordinated Universal Time (UTC), including the `Z` designator. Example: `2025-05-16T15:30:00.123Z`.
  ...
  "mcpServers": {
    "conduit_mcp": { // Client-defined alias for this server instance
      ...
      "env": { // Environment variables passed to the server process
        "LOG_LEVEL": "INFO", // For server's internal logging detail if captured elsewhere
        "CONDUIT_ALLOWED_PATHS": "~/.my_agent_data:/projects" // Example explicit user setting
        // If CONDUIT_ALLOWED_PATHS is omitted, server defaults to "~:/tmp"
      }
    }
  }
}
```
*Documentation must prominently explain the `CONDUIT_ALLOWED_PATHS` variable, its default (`~:/tmp`), and the one-time informational notice sent via the first MCP response if the default is used. It should strongly advise users to set this variable explicitly for production environments.*
    ...
    ```json
    {
      "mcpServers": {
        "conduit_mcp_local": {
          ...
          "env": {
            "LOG_LEVEL": "DEBUG",
            // CONDUIT_ALLOWED_PATHS is omitted here to test default behavior,
            // or explicitly set: "CONDUIT_ALLOWED_PATHS": "~",
...
The server **must** parse these environment variables at startup. String values for numbers and booleans **must** be robustly parsed. The server **must** resolve `~` at the beginning of any path segment in the `CONDUIT_ALLOWED_PATHS` string to the current user's home directory (e.g., using `os.homedir()` from Node.js `os` module) before using the paths.

*   **`LOG_LEVEL`**: `string` (Default: `"INFO"`). For internal server logging if its output is ever enabled (not to MCP client). Valid values (case-insensitive): `"TRACE"`, `"DEBUG"`, `"INFO"`, `"WARN"`, `"ERROR"`, `"FATAL"`.
...
*   **`CONDUIT_RECURSIVE_SIZE_TIMEOUT_MS`**: `string` (Integer, Default: `"60000"` - 60 seconds). Internal server-side timeout for potentially long-running `calculate_recursive_size` operations. This is not configurable per MCP request.

### 5. Tools Provided

*(The AI building the server is responsible for generating extensive descriptions for the project's README.md for each tool, and each of its operations/actions. These descriptions must be inferred from the detailed parameter specifications, return structures, default behaviors, and error conditions outlined below. Each description should clearly state the purpose, all parameters with their types, optionality, and defaults, the structure of the success response with all possible fields, common error codes, and include 1-2 illustrative JSON examples of usage.)*

#### 5.1. Tool: `read`

*   **Description:** (AI to generate: Example start - "The `read` tool is a versatile data retrieval utility. It allows fetching content from local files (within configured allowed paths) and remote URLs. It can also retrieve detailed metadata for these sources, calculate cryptographic checksums of their content, and compare the differences between two specified local files. Key features include support for partial content reads (byte ranges) for both local files and URLs, automatic image compression for large images when requested in base64 format, and intelligent conversion of fetched HTML webpages into cleaned Markdown. The tool provides smart defaults for output formats based on content type but allows explicit format requests...")
*   **Parameters:**
    ...
    *   **Parameters specific to `operation: "content"`:**
        *   `format?`: `string` (Optional). Specifies the desired output format for the content.
            ...
            *   **Specific `format` behavior details:**
                *   `"text"`: Returns content as a UTF-8 string. If explicitly requested for content identified as binary (based on MIME type as described above for local files, or non-text `Content-Type` for URLs), the `content` field in the response **must** be the placeholder string: `"[Binary content, request with format: 'base64' to view]"`.
                ...
                *   `"markdown"`: **Primarily for URL sources.**
                    *   If the source is a URL and its `Content-Type` is HTML (e.g., `text/html`), the server performs the webpage cleaning pipeline (fetch, parse, main content extraction via Readability, convert main HTML to Markdown via Turndown).
                    ...
                    *   If the source is a local file, requesting `format: "markdown"` for a local HTML file should perform the same cleaning pipeline. For non-HTML local files, it should behave like the non-HTML URL case (fallback to text, include skip reason).
        ...
        *   `offset?`: `integer` (Optional, default: `0`). Byte offset from which to start reading. Must be non-negative.
        *   `length?`: `integer` (Optional, default: `-1`, which means read from `offset` to the end of the content). Number of bytes to read. Must be non-negative if specified (other than -1).
...
*   **Returns:**
    ...
    *   *(The first successful response of a server session may prepend the one-time informational notice object to the primary response array, or wrap the single object response in an array with the notice as the first element, if default paths were used by the server. Clients should be prepared for this possibility on the first call.)*
    ...
    *   **Additional fields for `operation: "metadata"` on `status: "success"`:**
        ...
        *   `metadata`: `object` (Contains metadata attributes):
            *   `name`: `string` (Filename for local files, or the last segment of the URL path for URLs).
            ...
            *   `http_headers?`: `object` (For `source_type: "url"`, a key-value map of *all* HTTP response headers received from the remote server. Header names should be lowercased for consistency).
    *   **Additional fields for `operation: "diff"` on `status: "success"`:**
        ...
        *   `diff_content`: `string` (The textual representation of the differences).

#### 5.2. Tool: `write`

*   **Description:** (AI to generate: Example start - "The `write` tool facilitates modifications to the local filesystem within configured allowed paths. It supports creating or overwriting files with specified content (text or base64 encoded), appending content to existing files, creating new directories (recursively if needed), and managing the file/directory structure through copy, move, and delete operations. It also allows updating file timestamps ('touch') and creating or extracting standard archive files (zip, tar.gz). Most individual file and directory manipulation actions can be batched in a single request for improved efficiency. Operations are performed on a best-effort basis for each item in a batch; atomicity across multiple entries is not guaranteed...")
*   **Parameters:**
    *   `action`: `string` (Required). Valid values: `"put" | "mkdir" | "copy" | "move" | "delete" | "touch" | "archive" | "unarchive"`.
    *   `entries?`: `object[]` (Required for actions: `"put"`, `"mkdir"`, `"copy"`, `"move"`, `"delete"`, `"touch"`. This array **must not** be empty for these actions. If it is missing or empty, the server **must** return an `ERR_INVALID_PARAMETER` error detailing that `entries` is required and cannot be empty). Each object within the `entries` array defines a single operation with its specific parameters:
        *   **Parameters for each entry in `action: "put"`:**
            ...
            *   `input_encoding?`: `string` (Optional, default: `"text"`). Valid values: `"text"` (content is UTF-8 string) or `"base64"` (content is a base64 encoded string, which will be decoded to binary before writing).
            *   `write_mode?`: `string` (Optional, default: `"overwrite"`). Valid values: `"overwrite"` (creates a new file or overwrites an existing one) or `"append"` (adds content to the end of an existing file; creates the file if it does not exist).
        *   **Parameters for each entry in `action: "mkdir"`:**
            *   `path`: `string` (Required). The local filesystem path of the directory to create.
            *   `recursive?`: `boolean` (Optional, default: `false`). If `true`, parent directories will be created if they do not exist (similar to `mkdir -p`). This operation is idempotent.
        *   **Parameters for each entry in `action: "copy"`:**
            *   `source_path`: `string` (Required). The path to the source file or directory.
            *   `destination_path`: `string` (Required). The path to the destination. If `destination_path` is an existing directory, `source_path` (file or directory) is copied *inside* it. If `source_path` is a directory, its contents are copied recursively. Overwrites existing files at the destination by default.
        *   **Parameters for each entry in `action: "move"`:**
            *   `source_path`: `string` (Required).
            *   `destination_path`: `string` (Required). If `destination_path` is an existing directory, `source_path` is moved *inside* it. Otherwise, `source_path` is renamed/moved to `destination_path`. Overwrites existing files at the destination by default.
        *   **Parameters for each entry in `action: "delete"`:**
            *   `path`: `string` (Required). Path to the file or directory to delete.
            *   `recursive?`: `boolean` (Optional, default: `false`). **Must be `true`** to delete a non-empty directory. This parameter is ignored if `path` points to a file.
        *   **Parameters for each entry in `action: "touch"`:**
            *   `path`: `string` (Required). Creates an empty file if it doesn't exist, or updates both access and modification timestamps to the current server time if it does exist. Idempotent.
    *   **Parameters specific to `action: "archive"` (This is a single operation, not batched via `entries` field):**
        *   `source_paths`: `string[]` (Required). An array of one or more local file or directory paths to be included in the archive. If any path in this array does not exist or is inaccessible, it is skipped, and a note about these skipped paths is included in the success response.
        ...
        *   `format?`: `string` (Optional, default: `"zip"`). Valid values: `"zip" | "tar.gz" | "tgz"` (`"tgz"` is an alias for `"tar.gz"`).
        *   `recursive_source_listing?`: `boolean` (Optional, default: `true`). If `true` and a path in `source_paths` is a directory, its contents are added recursively to the archive, preserving the internal directory structure relative to that source directory.
    *   **Parameters specific to `action: "unarchive"` (This is a single operation, not batched via `entries` field):**
        ...
        *   `destination_path`: `string` (Required). The local directory path where the contents of the archive will be extracted. This directory will be created if it doesn't already exist.
...
*   **Returns:**
    ...
    *   **Common fields for each result object:**
        ...
        *   `path?`: `string` (The primary path involved in the operation, e.g., for `put`, `mkdir`, `delete`, `touch`. For `archive`, this is `archive_path`. For `unarchive`, this is `archive_path`).
        *   `source_path?`: `string` (For `copy`, `move` actions, the source path for that specific entry).
    ...
    *   **Additional fields on `status: "success"`:**
        ...
        *   `skipped_sources?`: `string[]` (For `action: "archive"`, an array of source paths that were skipped because they didn't exist or were inaccessible. Omitted if no sources were skipped).
        *   `extracted_files_count?`: `integer` (For `action: "unarchive"`, the number of files/directories successfully extracted. Omitted if not applicable or extraction failed).

#### 5.3. Tool: `list`

*   **Description:** (AI to generate: Example start - "The `list` tool provides capabilities to inspect local directory structures and retrieve system-level information about the server and its environment. The `entries` operation allows listing files and subdirectories within a specified path, with options for recursive traversal to a configured maximum depth and calculation of the total size of all contained files within directories. The `system_info` operation provides details about the server's capabilities, active configuration, or statistics for a specified filesystem volume...")
*   **Parameters:**
    ...
    *   **Parameters specific to `operation: "entries"`:**
        ...
        *   `recursive_depth?`: `integer` (Optional, default: `0`).
            ...
            *   Any value greater than `CONDUIT_MAX_RECURSIVE_DEPTH` or `-1` will be effectively capped by `CONDUIT_MAX_RECURSIVE_DEPTH`.
        *   `calculate_recursive_size?`: `boolean` (Optional, default: `false`). If `true`, for each directory entry (including the top-level `path` itself if it's a directory being listed with `recursive_depth:0` or greater), its `size_bytes` field will attempt to be the sum of all files within it and its subdirectories (respecting the `recursive_depth` and subject to `CONDUIT_RECURSIVE_SIZE_TIMEOUT_MS`).
...
*   **Returns:**
    *   **For `operation: "entries"`:** An array of `EntryInfo` objects.
        *   Each `EntryInfo` object:
            ...
            *   `size_bytes?`: `integer` (Size of the file. For directories: if `calculate_recursive_size` is `false`, this is the OS-reported directory size (often small). If `calculate_recursive_size` is `true`, this is the sum of contained file sizes. Will be `null` if calculation timed out, failed, or was not applicable for that entry, accompanied by `recursive_size_calculation_note`).
    ...
    *   **For `operation: "system_info"` with `info_type: "server_capabilities"`:** A single object:
        *   `server_version`: `string` (e.g., "1.0.0").
        *   `active_configuration`: `object` (A map of active `CONDUIT_*` settings, with paths resolved and sensitive values appropriately handled if any were deemed so - though currently none are. E.g., `CONDUIT_ALLOWED_PATHS` should be an array of resolved strings).
    ...
    *   **For `operation: "system_info"` with `info_type: "filesystem_stats"`:**
        *   **If `path` parameter is provided, valid, and within an allowed path:** A single object:
            ...
            *   `free_bytes`: `integer` (Free space available on the volume).
            *   `available_bytes`: `integer` (Free space available to the current user, might be less than `free_bytes`).
            *   `used_bytes`: `integer` (Used space on the volume).
        *   **If `path` parameter is NOT provided:** A single informational object:
            ```json
            {
              ...
              "server_version": "1.0.0", // Actual server version
              "server_start_time_iso": "YYYY-MM-DDTHH:mm:ss.sssZ", // Actual server start time
              "configured_allowed_paths": ["/actual/resolved/path/one", "/actual/resolved/path/two"] // Current list from CONDUIT_ALLOWED_PATHS (resolved)
            }
            ```
    *   *(The first successful response of a server session may prepend the one-time informational notice object if default paths were used. Clients should handle this possibility.)*
...
*   **Description:** (AI to generate: Example start - "The `find` tool enables powerful and flexible searching for files and directories within the configured allowed local filesystem paths. Searches can be initiated from a specified base path, with options for recursive traversal up to a defined depth. It allows for complex queries by combining multiple criteria using AND logic. Search criteria can include glob patterns for names, literal text or regular expression matching for file content, and filters based on a wide range of metadata attributes like size, creation/modification timestamps, entry type (file or directory), and MIME type...")
*   **Parameters:**
    *   `base_path`: `string` (Required). The local directory path from which the search will originate.
    *   `recursive?`: `boolean` (Optional, default: `true`). If `true`, the search will extend into subdirectories, respecting `CONDUIT_MAX_RECURSIVE_DEPTH`. If `false`, only the `base_path` itself is searched (for entries directly within it, not its content if `base_path` is a file).
    *   `match_criteria`: `object[]` (Required). An array of criterion objects. An entry is included in the results only if it **matches ALL criteria** defined in this array (implicit AND logic between criterion objects).
        *   Each criterion object **must** have a `type` field, and other fields depending on the `type`:
            *   **`type: "name_pattern"`**
                *   `pattern`: `string` (Required). A glob pattern (e.g., `*.txt`, `image[0-9]?.png`, `**/*.log`) to match against entry names (filenames or directory names).
            *   **`type: "content_pattern"`**
                *   `pattern`: `string` (Required). The text or regular expression pattern to search for within the content of files.
                *   `is_regex?`: `boolean` (Optional, default: `false`). If `true`, the `pattern` string is treated as a JavaScript-compatible regular expression. If `false`, `pattern` is treated as a literal string to be found.
                *   `case_sensitive?`: `boolean` (Optional, default: `false`). If `is_regex: false`, this controls case sensitivity of the literal string search. If `is_regex: true`, this flag is typically ignored as case sensitivity is controlled by regex flags (e.g., `/pattern/i`), but the server may choose to apply it if no flags are in the regex.
                *   `file_types_to_search?`: `string[]` (Optional). An array of file extensions (e.g., `[".txt", ".log", ".md"]`) to restrict content searching to these types of files. If omitted, the server will attempt to search content only in files that are presumed to be text-based (determined by checking against a list of common text file extensions and then by MIME type detection via `file-type` for text-friendly types like `text/*`, `application/json`, etc.). Files identified as binary (e.g., based on MIME type or falling outside common text extensions) **will always be skipped for content searching, regardless of whether their extension is listed in `file_types_to_search`**. If a file is skipped for content search due to being binary, no error is generated for that file, it simply will not match a `content_pattern` criterion.
            *   **`type: "metadata_filter"`**
                ...
                *   `value`: `any` (Required). The value to compare against. Its type must be appropriate for the `attribute`:
                    *   `string` for `name`, `entry_type`, `mime_type`. For `operator: "matches_regex"`, `value` is a regex string.
                    *   `integer` for `size_bytes`.
                    *   `string` (ISO 8601 date or datetime for `before`/`after`, `YYYY-MM-DD` for `on_date`) for date attributes.
                *   `case_sensitive?`: `boolean` (Optional, default: `false`). Applies to string attribute operators (`equals`, `contains`, `starts_with`, `ends_with`). Ignored if `operator` is `matches_regex` (regex flags control case sensitivity).
    *   `entry_type_filter?`: `string` (Optional, default: `"any"`). A shorthand filter for entry type. Valid values: `"file" | "directory" | "any"`. If specified, acts as an additional AND criterion.
