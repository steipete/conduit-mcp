...
import { createTempDir, cleanupAllTemp, getTempBasePath, tempFileExists, readTempFile } from './utils/tempFs';
...
describe('E2E Write Operations', () => {
  ...
  describe('Batch Write Operations', () => {
    it('should successfully write multiple files in batch', async () => {
      ...
      // Check each result
      result.response.results.forEach((writeResult: any, index: number) => {
        expect(writeResult.status).toBe('success');
        expect(writeResult.path).toBe(index === 0 ? file1 : file2);
      });
    ...
    it('should handle mixed success and failure in batch operations', async () => {
      ...
      expect(secondResult.error_message).toMatch(/ENOENT|no such file or directory|Failed to write/i);
  ...
  describe('Error Handling', () => {
    it('should handle access denied for paths outside allowed paths', async () => {
      ...
      expect(result.response.results[0].error_message).toMatch(/ENOENT|no such file or directory|Failed to write/i);
