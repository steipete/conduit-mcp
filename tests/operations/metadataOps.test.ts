  ...
  describe('getMetadata - for File source', () => {
    ...
    it('should return success with file metadata for a valid file path', async () => {
      ...
      mockedFsOps.createEntryInfo.mockResolvedValue({
        ...
        created_at_iso: '2023-01-01T10:00:00.000Z',
        modified_at_iso: '2023-01-02T11:00:00.000Z',
        permissions_octal: '0644',
        permissions_string: '-rw-r--r--',
      });

      const result = await getMetadata(testFilePath, baseFileParams, mockedConfig as ConduitServerConfig) as ReadTool.MetadataResultSuccess;
      ...
      expect(result.metadata.created_at_iso).toBe('2023-01-01T10:00:00.000Z');
      expect(result.metadata.modified_at_iso).toBe('2023-01-02T11:00:00.000Z');
  ...
  describe('getMetadata - for URL source', () => {
    const baseWebParams: ReadTool.MetadataParams = {
      ...
      it('should return success with URL metadata for a valid URL', async () => {
        const mockFetchContentResult = { // Renamed for clarity
        ...
        const result = await getMetadata(testFileUrl, baseWebParams, mockedConfig as ConduitServerConfig) as ReadTool.MetadataResultSuccess;
        ...
        expect(result.metadata.modified_at_iso).toBe('2023-01-03T12:00:00.000Z');
