...
vi.mock('@/internal', async (importOriginal) => {
  ...
  return {
    ...
    logger: loggerMock,
    configLoader: configLoaderMockObject,
...
describe('getContentOps', () => {
  ...
  const mockedGetCurrentISO8601UTC = getCurrentISO8601UTC as MockedFunction<typeof getCurrentISO8601UTC>;
  ...
  beforeEach(() => {
    ...
    // Reset the conduitConfig part of the mockedConfigLoader
    // The mockedConfig alias points to configLoader.conduitConfig
  ...
  describe('from File source (via getContent)', () => {
    ...
    const baseParams: ReadTool.ContentParams = { // This baseParams is for file tests
      operation: 'content',
      sources: [filePath],
    };
    ...
    it('should return error if source is a directory', async () => {
      ...
      const errorResult = result as (MCPErrorStatus & { source: string, source_type: 'file' | 'url' });
    ...
    it('should default to "text" format for text-like MIME types', async () => {
      ...
      const params: ReadTool.ContentParams = { ...baseParams };
      const result = await getContent(filePath, params, mockedConfig) as ReadTool.ContentResultSuccess; 

      expect(result.status).toBe('success');
      expect(result.output_format_used).toBe('text');
      expect(result.content).toBe('Hello world');
      expect(mockedFsOps.readFileAsBuffer).toHaveBeenCalledWith(filePath, mockedConfig.maxFileReadBytes);
    });
    ...
    it('should handle "checksum" format', async () => {
      ...
      const params: ReadTool.ContentParams = { ...baseParams, format: 'checksum', checksum_algorithm: 'sha1' };
      const result = await getContent(filePath, params, mockedConfig) as ReadTool.ContentResultSuccess; 
      ...
      expect(mockedCalculateChecksum).toHaveBeenCalledWith(Buffer.from('some data for checksum'), 'sha1');
    });
    ...
    it('should handle "checksum" format with default algorithm', async () => {
        ...
        expect(mockedCalculateChecksum).toHaveBeenCalledWith(Buffer.from('data for default checksum'), 'md5');
    ...
    it('should handle range requests for "text" format', async () => {
      ...
      const mockFileHandle = {
        read: vi.fn().mockImplementation(async (bufferToFill, bufferOffset, bytesToRead, filePosition) => {
          const sourceBuffer = Buffer.from(expectedSubstring);
          sourceBuffer.copy(bufferToFill, bufferOffset, 0, Math.min(bytesToRead, sourceBuffer.length));
          return { bytesRead: Math.min(bytesToRead, sourceBuffer.length), buffer: bufferToFill };
        }),
        close: vi.fn().mockResolvedValue(undefined),
      };
      (fsPromises.open as MockedFunction<typeof fsPromises.open>).mockResolvedValueOnce(mockFileHandle as any);
    ...
    it('should handle range requests for "base64" format', async () => {
      ...
      const mockFileHandle = {
        read: vi.fn().mockImplementation(async (bufferToFill, bufferOffset, bytesToRead, filePosition) => {
          expectedSlice.copy(bufferToFill, bufferOffset, 0, Math.min(bytesToRead, expectedSlice.length));
    ...
    it('should return error if fs.stat fails', async () => {
      ...
      const conduitStatError = new ConduitError(ErrorCode.OPERATION_FAILED, expectedConduitErrorMessage);
    ...
    it('should return error if readFileAsBuffer fails for base64 format', async () => {
        ...
        const conduitReadError = new ConduitError(ErrorCode.ERR_FS_READ_FAILED, "Failed to read file due to EIO");
    ...
    it('should apply image compression for image MIME types and base64 format', async () => {
        ...
        mockedFsOps.getStats.mockResolvedValueOnce({
          isFile: () => true, isDirectory: () => false, size: 1000,
        } as fs.Stats);
        ...
        (mockedConfig as any).imageCompressionThresholdBytes = defaultTestConfig.imageCompressionThresholdBytes;
      });
  ...
  describe('from URL source (via getContent)', () => {
    ...
    const baseParamsUrl: ReadTool.ContentParams = { // Renamed to avoid conflict
      operation: 'content',
      sources: [testUrl],
    };

    it('should successfully fetch text content from a URL', async () => {
      const mockFetchedContent: webFetcher.FetchedContent = { // Corrected type usage
        finalUrl: testUrl,
      ...
      const result = await getContent(testUrl, params, mockedConfig) as ReadTool.ContentResultSuccess;
    ...
    it('should default to "base64" format for non-text-like MIME types from URL', async () => {
      const mockFetchedContent: webFetcher.FetchedContent = {
        finalUrl: testUrl,
    ...
    it('should handle "checksum" format for URL content with specified algorithm', async () => {
      ...
      const params: ReadTool.ContentParams = { ...baseParamsUrl, format: 'checksum', checksum_algorithm: 'sha1' };
      const result = await getContent(testUrl, params, mockedConfig) as ReadTool.ContentResultSuccess;
      ...
      expect(mockedCalculateChecksum).toHaveBeenCalledWith(Buffer.from('some url data for checksum'), 'sha1');
      expect(mockedWebFetcher.fetchUrlContent).toHaveBeenCalledWith(testUrl, false, undefined);
    });
    ...
    it('should handle "checksum" format for URL content with default algorithm', async () => {
      ...
      expect(mockedCalculateChecksum).toHaveBeenCalledWith(Buffer.from('data for url default checksum'), 'sha512');
      (mockedConfig as any).defaultChecksumAlgorithm = defaultTestConfig.defaultChecksumAlgorithm;
    });
    ...
    it('should simulate range if no range header sent but params specify range', async () => {
      ...
      expect(mockedWebFetcher.fetchUrlContent).toHaveBeenCalledWith(testUrl, false, expectedRangeHeader);
    });

    it('should return error if webFetcher.fetchUrlContent throws ConduitError for HTTP status', async () => {
      const fetchError = new ConduitError(ErrorCode.ERR_HTTP_STATUS_ERROR, 'Request to http://example.com/test.txt failed with HTTP status 404. Message: Not Found');
      ...
      const errorResult = result as (MCPErrorStatus & { source: string, source_type: 'file' | 'url', http_status_code?: number });
      expect(errorResult.error_code).toBe(ErrorCode.ERR_HTTP_STATUS_ERROR);
      expect(errorResult.error_message).toBe('Request to http://example.com/test.txt failed with HTTP status 404. Message: Not Found');
    ...
    describe('Markdown conversion from URL', () => {
      ...
      it('should successfully convert HTML from URL to Markdown', async () => {
        ...
        const params: ReadTool.ContentParams = { ...baseParamsUrl, sources: [htmlUrl], format: 'markdown' };
        const result = await getContent(htmlUrl, params, mockedConfig) as ReadTool.ContentResultSuccess;
      ...
      it('should return error if cleanHtmlToMarkdown throws an error', async () => {
        ...
        mockedWebFetcher.fetchUrlContent.mockResolvedValueOnce(mockFetchedContent);
        const markdownError = new ConduitError(ErrorCode.ERR_MARKDOWN_CONVERSION_FAILED, 'Markdown conversion boom');
        mockedWebFetcher.cleanHtmlToMarkdown.mockImplementationOnce(() => { throw markdownError; });
      ...
      it('should handle non-HTML content type when Markdown is requested', async () => {
        ...
        const params: ReadTool.ContentParams = { ...baseParamsUrl, sources: [nonHtmlUrl], format: 'markdown' };
        const result = await getContent(nonHtmlUrl, params, mockedConfig) as ReadTool.ContentResultSuccess;
        ...
        expect(result.markdown_conversion_skipped_reason).toContain('Content type application/json is not HTML');
