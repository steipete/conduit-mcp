...
function createErrorMkdirResultItem(path: string | undefined, errorCode: ErrorCode, errorMessage: string): WriteTool.WriteResultItem {
    return {
        status: 'error',
...
export async function makeDirectory(
    ...
    if (!entry.path) {
        return createErrorMkdirResultItem(entry.path /*undefined*/, ErrorCode.INVALID_PARAMETER, 'path is required for mkdir.');
    }
    ...
    try {
        ...
        if (pathExists) {
            ...
            if (stats.isDirectory()) {
                ...
                return createErrorMkdirResultItem(entry.path, ErrorCode.ERR_FS_PATH_IS_FILE, `Path exists but is a file, not a directory: ${entry.path}`);
            }
        }
        ...
        if (error instanceof ConduitError) {
            ...
            if (error.errorCode === ErrorCode.ACCESS_DENIED) {
                return createErrorMkdirResultItem(entry.path, ErrorCode.ERR_FS_PERMISSION_DENIED, `Permission denied for path: ${entry.path}`);
            }
            // A component of the path prefix is not a directory, or other fs errors
            if (error.errorCode === ErrorCode.ERR_FS_NOT_FOUND || 
                (error.message && (error.message.includes('ENOTDIR') || error.message.includes('Not a directory')))) {
                return createErrorMkdirResultItem(entry.path, ErrorCode.ERR_FS_PATH_IS_FILE, `A component of the path prefix is not a directory: ${entry.path}`);
            }
        ...
        if (error.code === 'EEXIST') { // Should be caught by pathExists check, but as a safeguard
            return createErrorMkdirResultItem(entry.path, ErrorCode.RESOURCE_ALREADY_EXISTS, `Path already exists (unexpectedly): ${entry.path}`);
        }
        if (error.code === 'ENOTDIR') {
            return createErrorMkdirResultItem(entry.path, ErrorCode.ERR_FS_PATH_IS_FILE, `A component of the path prefix is not a directory: ${entry.path}`);
        }
        // Generic fallback
        return createErrorMkdirResultItem(entry.path, ErrorCode.OPERATION_FAILED, `Failed to create directory ${entry.path}: ${error.message || 'Unknown error'}`);
    }
} 