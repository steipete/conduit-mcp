export interface MCPRequest {
  ...
  toolName: string;
  parameters: Record<string, unknown>; // Or a more specific union of all possible tool params
}

export interface MCPResponse {
  requestId?: string; // Should mirror the request's ID if provided
  response: unknown; // This will be specific to the tool's response structure
